...
struct APIManager: APIManagerProtocol {
    internal func request<T: Decodable>(url: String, completion: @escaping (_ result: [T]?, _ error: CustomError?) -> ()) {
        ...
        Alamofire.request(url, method: .get, headers: headers).responseJSON() { response in
            ...
            guard let genericCollection = try? decoder.decode([T].self, from: jsonData) else { return completion(nil, try? decoder.decode(CustomError.self, from: jsonData))}
            return completion(genericCollection, nil)
        }
    }
}
